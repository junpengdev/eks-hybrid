...
func (c *NodeCreate) WaitForNodeReady(ctx context.Context, verifyNode *kubernetes.VerifyNode, peeredNetwork *Network, peeredNode *PeerdNode) error {
	_, err := verifyNode.WaitForNodeReady(ctx)
	if err != nil {
		return fmt.Errorf("waiting node object in kubernetes to be ready: %w", err)
	}

	// Configure PodCIDR for EC2 route table
	if err := peeredNetwork.CreateRoutesForNode(ctx, peeredNode); err != nil {
		return fmt.Errorf("creating routes for peered node: %w", err)
	}
